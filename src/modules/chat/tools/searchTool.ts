...
export async function executeSearchLibrary(
    ...
    try {
        ...
        Zotero.debug(`[seerai] Tool: search_library query="${query}" limit=${effectiveLimit}`);
        Zotero.debug(`[seerai] Tool: search_library config scope=${JSON.stringify(config.libraryScope)}`);
        ...
        if (config.libraryScope.type === "user") {
            ...
            searchLibraryID = typeof groupLibId === 'number' ? groupLibId : Zotero.Libraries.userLibraryID;
            ...
            searchLibraryID = colScope.libraryID !== undefined ? colScope.libraryID : Zotero.Libraries.userLibraryID;
        } else {
        ...
        if (query) {
            ...
            if (itemIDs.length < effectiveLimit) {
                ...
                Zotero.debug(`[seerai] Title search found: ${titleResults.length} items`);
            ...
            if (itemIDs.length === 0 && query.includes(" ")) {
                ...
                if (words.length > 1) {
                    Zotero.debug(`[seerai] No results for full query, trying keyword search: ${words.slice(0, 3).join(", ")}`);
                    const kwSearch = createSearch(searchLibraryID);
                    // Join with AND
                    for (const word of words.slice(0, 5)) { // Use up to 5 words
        ...
        if (searchCollectionID !== undefined) {
            ...
            if (collection) {
                ...
                Zotero.debug(`[seerai] Collection filter applied: ${itemIDs.length} items remain`);
            }
        }

        // Apply Filters (if specified in tool call)
        let filteredItems: Zotero.Item[] = [];
        for (const id of itemIDs) {
            ...
            if (filters) {
                ...
                if (filters.year_from || filters.year_to) {
                    ...
                    if (filters.year_from && (!year || year < filters.year_from)) continue;
        ...
        for (const item of limitedItems) {
            ...
            const authors = creators.map((c: any) =>
                c.lastName ? `${c.firstName || ""} ${c.lastName}`.trim() : c.name || ""
            ).filter(Boolean);
            ...
            const abstractPreview = abstract.length > 200
...
export async function executeSearchExternal(
    ...
    try {
        ...
        Zotero.debug(`[seerai] Tool: search_external query="${query}" limit=${effectiveLimit}`);
