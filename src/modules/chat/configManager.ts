...
import { loadPrompts, savePrompts, getAllPromptTags, PromptTemplate, getDefaultTemplates } from "./promptLibrary";
...
function getPrefKeys(branchName: string): string[] {
    try {
        ...
        Zotero.debug(`[seerai] Error getting preference branch ${branchName}: ${e}`);
        return [];
    }
}

// Branches to export
const PREFERENCE_BRANCHES = [
    "extensions.seerai.",
    "extensions.zotero.seerai."
];
 ...
 */
export async function importAllData(data: SeerAIConfig): Promise<{ success: boolean; stats: string; error?: string }> {
    try {
        if (!data || !data.meta || data.meta.version !== CONFIG_VERSION) {
            // Basic version check, potentially handle migrations here in future
            Zotero.debug(`[seerai] Config version mismatch or invalid format. Expected ${CONFIG_VERSION}, got ${data?.meta?.version}`);
            ...
            if (data?.meta?.version > CONFIG_VERSION) {
                throw new Error(`Unsupported configuration version: ${data.meta.version}`);
            }
        }
        ...
        if (data.preferences) {
            for (const [key, value] of Object.entries(data.preferences)) {
                ...
                if (key.startsWith("extensions.seerai.") || key.startsWith("extensions.zotero.seerai.")) {
